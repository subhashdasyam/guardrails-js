...
function cacheDir() {
  const base = process.env.CLAUDE_PLUGIN_DATA || path12.join(os3.homedir(), ".claude", "plugins", "data", "guardrails-js");
  return path12.join(base, "cache");
}
function cacheFile(key) {
  const safe = String(key).replace(/[^A-Za-z0-9_.@-]/g, "-");
  return path12.join(cacheDir(), `${safe}.json`);
}
function readCache(key, now) {
  try {
    const raw = JSON.parse(fs10.readFileSync(cacheFile(key), "utf8"));
...
function writeCache(key, value, now) {
  try {
    ...
    fs10.writeFileSync(temp, JSON.stringify({ at: now, value }), "utf8");
...
function readLockedVersions(projectRoot) {
  ...
  for (const file of ["package-lock.json", "npm-shrinkwrap.json"]) {
    let lock2;
    try {
      lock2 = JSON.parse(fs5.readFileSync(path5.join(projectRoot, file), "utf8"));
    } catch {
      continue;
    }
...
function dataRoot() {
  return path6.join(process.env.CLAUDE_PLUGIN_DATA || path6.join(os2.homedir(), ".claude/plugins/data/guardrails-js"), "threat");
}
function pluginRoot() {
  let dir = path6.dirname(fileURLToPath(import.meta.url));
  for (let i = 0; i < 5; i++) {
    if (fs6.existsSync(path6.join(dir, "hooks/hooks.json"))) return dir;
...
function writeJson(file, data) {
  ...
  const temp = `${file}.${process.pid}.tmp`;
  fs6.writeFileSync(temp, JSON.stringify(data, null, 2) + "\n");
  fs6.renameSync(temp, file);
}
function snapshot() {
  if (process.env.GUARDRAILS_THREAT_SNAPSHOT) {
    const dir2 = path6.resolve(process.env.GUARDRAILS_THREAT_SNAPSHOT);
    return { file: path6.join(dir2, "threat-data.db"), metadata: readJson2(path6.join(dir2, "manifest.json")) };
  }
  const root = dataRoot();
  const active = readJson2(path6.join(root, "active.json"));
  if (active && /^[a-f0-9]{64}$/.test(active.sha256)) {
    const file = path6.join(root, "snapshots", active.sha256, "threat-data.db");
    if (fs6.existsSync(file)) return { file, metadata: active };
  }
  const dir = path6.join(pluginRoot(), "data");
  return { file: path6.join(dir, "threat-data.db"), metadata: readJson2(path6.join(dir, "manifest.json")) };
}
function lock(file, leaseMs = 20 * 6e4) {
  fs6.mkdirSync(path6.dirname(file), { recursive: true });
  try {
    if (Date.now() - fs6.statSync(file).mtimeMs > leaseMs) fs6.unlinkSync(file);
  } catch {
  }
  try {
    const fd = fs6.openSync(file, "wx");
    fs6.writeFileSync(fd, String(process.pid));
    fs6.closeSync(fd);
    return () => {
      try {
        fs6.unlinkSync(file);
...
function driverDirectory() {
  if (process.env.GUARDRAILS_SQLITE_RUNTIME) return path7.resolve(process.env.GUARDRAILS_SQLITE_RUNTIME);
  return path7.join(dataRoot(), "runtime", `${process.platform}-${process.arch}`, SQLITE3_VERSION);
}
function nativeDriver() {
  return createRequire(path7.join(driverDirectory(), "package.json"))("sqlite3");
}
var builtin;
...
async function lookupThreats(names, file = snapshot().file) {
  ...
  try {
    ...
    try {
      fs7.unlinkSync(path8.join(dataRoot(), "store-error.json"));
    } catch {
    }
  } catch (error) {
    try {
      writeJson(path8.join(dataRoot(), "store-error.json"), { at: Date.now(), error: error.message });
    } catch {
    }
...
function isManifestFile(filePath) {
  const base = path9.basename(String(filePath));
...
function isMiddlewareFile(filePath) {
  const base = path10.basename(String(filePath));
...
var NEST_PUBLIC = {
  ...
  match(node, ctx) {
    ...
    const path14 = (node.decorators ?? []).map((decorator) => staticString(decorator.expression?.arguments?.[0])).find(Boolean) ?? "";
    const sensitive = SENSITIVE_NAME.test(methodName) || SENSITIVE_NAME.test(path14);
...
function manualSteps() {
  const command = JSON.stringify(path11.join(pluginRoot(), "dist/threat-data.mjs"));
...
function maintainThreatData(config, { now = Date.now(), launch = spawn } = {}) {
  ...
  try {
    const root = dataRoot();
    const state = readJson2(path11.join(root, "update-state.json"), {});
    ...
    if (config.network !== false && config.threatDataAutoRefresh !== false && now >= (state.nextCheck || 0)) {
      const release2 = lock(path11.join(root, "launch.lock"), 6e4);
      if (release2) {
        try {
          const last = readJson2(path11.join(root, "launch.json"), {});
          if (now - (last.at || 0) >= 6e4) {
            writeJson(path11.join(root, "launch.json"), { at: now });
            const worker = launch(process.execPath, [path11.join(pluginRoot(), "dist/threat-data.mjs"), "refresh", "--interval", String(interval)], {
              detached: true,
              stdio: "ignore",
              windowsHide: true,
              env: { ...process.env, CLAUDE_PLUGIN_DATA: path11.dirname(root) }
            });
            worker.on("error", () => {
            });
            worker.unref();
    ...
    const error = readJson2(path11.join(root, "store-error.json"));
    const problem = error && now - error.at < DAY ? `Threat database checks are unavailable: ${error.error}.` : !fs9.existsSync(current.file) || !Number.isFinite(age) ? "No usable threat database is installed." : age > 7 * DAY ? `Threat database is ${Math.floor(age / DAY)} days old (source snapshot ${current.metadata.sourceUpdated}).` : "";
    if (!problem) return "";
    const release = lock(path11.join(root, "notice.lock"), 6e4);
    if (!release) return "";
    try {
      const previous = readJson2(path11.join(root, "notice.json"), {});
      if (now - (previous.at || 0) < DAY) return "";
      writeJson(path11.join(root, "notice.json"), { at: now });
    } finally {
      release();
    }
...
function manifestPackage(projectRoot) {
  try {
    return JSON.parse(fs11.readFileSync(path13.join(projectRoot, "package.json"), "utf8"));
  } catch {
    return null;
  }
}
async function checkManifest(filePath, input) {
  const projectRoot = path13.dirname(filePath);
  const config = loadConfig(projectRoot);
  let pkg = null;
  if (path13.basename(filePath) === "package.json") {
    try {
      pkg = JSON.parse(fs11.readFileSync(filePath, "utf8"));
    } catch {
      return;
    }
  }
...
async function main() {
  ...
  let source;
  try {
    source = fs11.readFileSync(filePath, "utf8");
  } catch {
    return;
  }
  if (source.length > 2e6) return;
  const cwd = input.cwd || process.cwd();
  const config = loadConfig(path13.dirname(filePath));
  const { pkg, root } = readPackageJson(path13.dirname(filePath));
