...
async function fired(files) {
  return (await runManifestRules(project(files), config)).map((finding) => finding.ruleId);
}
...
test('every manifest rule is covered here', async () => {
...
test('a project doing everything right produces nothing', async () => {
  assert.deepEqual((await fired(CLEAN)), []);
});

test('SUPPLY-LOCK fires when there is no lockfile', async () => {
  const { 'package-lock.json': _dropped, ...noLock } = CLEAN;
  assert.ok((await fired(noLock)).includes('SUPPLY-LOCK'));
});

test('SUPPLY-LOCK fires when npmrc turns lockfile writing off', async () => {
  assert.ok(
    (await fired({ ...CLEAN, '.npmrc': 'ignore-scripts=true\npackage-lock=false\n' })).includes(
      'SUPPLY-LOCK',
    ),
  );
});

test('SUPPLY-LOCK stays quiet with a lockfile present', async () => {
  assert.ok(!(await fired(CLEAN)).includes('SUPPLY-LOCK'));
});

test('SUPPLY-LOCK stays quiet for a project with no dependencies at all', async () => {
  const bare = (await fired({ 'package.json': { name: 'bare', version: '1.0.0' }, '.npmrc': '' }));
  assert.ok(!bare.includes('SUPPLY-SCRIPTS'));
  assert.ok(!bare.includes('SUPPLY-PROV'));
});

test('SUPPLY-SCRIPTS fires when install scripts are not disabled', async () => {
  const { '.npmrc': _dropped, ...noNpmrc } = CLEAN;
  assert.ok((await fired(noNpmrc)).includes('SUPPLY-SCRIPTS'));
});

test('SUPPLY-SCRIPTS stays quiet when npmrc disables them', async () => {
  assert.ok(!(await fired(CLEAN)).includes('SUPPLY-SCRIPTS'));
});

test('SUPPLY-SCRIPTS stays quiet when a script passes the flag', async () => {
  const files = {
    ...CLEAN,
    '.npmrc': '',
    'package.json': {
  ...
  assert.ok(!(await fired(files)).includes('SUPPLY-SCRIPTS'));
});

test('SUPPLY-DENY fires on a known compromised release in the lockfile', async () => {
  const files = {
    ...CLEAN,
    'package-lock.json': {
      name: 'clean',
  ...
  const findings = (await runManifestRules(project(files), config));
...
test('SUPPLY-DENY fires on a bad exact version pinned in the manifest', async () => {
  const { 'package-lock.json': _dropped, ...files } = CLEAN;
  const withBad = {
    ...files,
    'package.json': { ...CLEAN['package.json'], dependencies: { debug: '4.4.2' } },
  };
  assert.ok((await fired(withBad)).includes('SUPPLY-DENY'));
});

test('SUPPLY-DENY stays quiet on a safe version of a listed package', async () => {
  const files = {
    ...CLEAN,
    'package-lock.json': {
      name: 'clean',
  ...
  assert.ok(!(await fired(files)).includes('SUPPLY-DENY'));
});

test('SUPPLY-DENY stays quiet on a package that was never compromised', async () => {
  assert.ok(!(await fired(CLEAN)).includes('SUPPLY-DENY'));
});

test('SUPPLY-PROV fires when nothing ever verifies signatures', async () => {
  const files = {
    ...CLEAN,
    'package.json': { ...CLEAN['package.json'], scripts: { test: 'node --test' } },
  };
  assert.ok((await fired(files)).includes('SUPPLY-PROV'));
});

test('SUPPLY-PROV stays quiet when a script verifies them', async () => {
  assert.ok(!(await fired(CLEAN)).includes('SUPPLY-PROV'));
});

test('SUPPLY-PROV stays quiet when CI verifies them', async () => {
  const files = {
    ...CLEAN,
    'package.json': { ...CLEAN['package.json'], scripts: { test: 'node --test' } },
...
test('findings carry the same shape as every other rule', async () => {
  const { '.npmrc': _dropped, ...noNpmrc } = CLEAN;
  const [finding] = (await runManifestRules(project(noNpmrc), config));
...
test('a disabled rule stays disabled', async () => {
  const off = loadConfig('/nonexistent');
  off.isRuleDisabled = (id) => id === 'SUPPLY-PROV';
  const ids = (await runManifestRules(project({ 'package.json': CLEAN['package.json'] }), off)).map(
    (finding) => finding.ruleId,
  );
  assert.ok(!ids.includes('SUPPLY-PROV'));
});

test('a directory with no package.json produces nothing', async () => {
  assert.deepEqual((await runManifestRules(project({ 'readme.md': '# hi' }), config)), []);
});

test('the files that trigger a recheck', async () => {
...
test('a compromised release blocks rather than prompting', async () => {
  const install = { subcommand: 'install', packages: ['chalk@5.6.1'], flags: [] };
  const verdict = (await evaluateInstall(install, { projectRoot: '/nope', known: new Set() }));
...
test('a clean package neither blocks nor prompts on the denylist signal', async () => {
...
test('allowPackages is the way past a block, since nothing else is', async () => {
  const install = { subcommand: 'install', packages: ['chalk@5.6.1'], flags: [] };

  const byVersion = (await evaluateInstall(install, {
    projectRoot: '/nope',
    known: new Set(),
  ...
  const byName = (await evaluateInstall(install, {
    projectRoot: '/nope',
    known: new Set(),
    allowPackages: ['chalk'],
  }));
...
test('allows matches a name or an exact version, and nothing else', async () => {
...
test('the severities that block are the ones with somewhere to upgrade to', async () => {
